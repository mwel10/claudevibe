...
The block above fetches from `main`, which moves. This one pins to a tag and
checks what arrived before any of it reaches `~/.claude`, which is the order
that matters: verifying after copying tells you what you already installed.

```bash
VER=v1.0.0
BASE=https://raw.githubusercontent.com/mwel10/claudevibe/$VER
STAGE=$(mktemp -d)
mkdir -p "$STAGE/hooks/lib" "$STAGE/agents"

curl --fail --remove-on-error -sSL -o "$STAGE/SHA256SUMS" "$BASE/SHA256SUMS"
while read -r _ F; do
  curl --fail --remove-on-error -sSL --create-dirs -o "$STAGE/$F" "$BASE/$F"
done < "$STAGE/SHA256SUMS"

(cd "$STAGE" && shasum -a 256 -c SHA256SUMS) || {
  echo "checksums do not match; nothing was installed"
  exit 1
}

mkdir -p ~/.claude/hooks/lib ~/.claude/agents
cp "$STAGE/global-CLAUDE.md"        ~/.claude/CLAUDE.md
cp "$STAGE/settings.json"           ~/.claude/settings.json
cp "$STAGE/hooks/"*.sh              ~/.claude/hooks/
cp "$STAGE/hooks/lib/deny-regex.py" ~/.claude/hooks/lib/
cp "$STAGE/agents/"*.md             ~/.claude/agents/
chmod +x ~/.claude/hooks/*.sh
```

Note the file list comes from `SHA256SUMS` itself, so the loop cannot fetch a
file the checksums do not cover, and cannot miss one they do.

Be clear about what this proves. It proves that what arrived is byte-for-byte
what that tag contains, so a truncated download, a proxy rewriting a file, or a
half-finished install stops being silent, and it proves you are installing a
fixed version rather than whatever `main` happens to be this morning. It does
not prove the tag is what I meant to publish: `SHA256SUMS` sits in the same tree
as the files it covers, so anyone able to write that tree can rewrite both. The
thing that would close that gap is a signed tag, and that is still an open item
in `CLAUDE.md` rather than something this repository does.

`SHA256SUMS` is generated by `scripts/checksums.sh` from the install block
above, so the set that is checksummed cannot drift from the set that is
installed, and CI fails if a shipped file changes without it being regenerated.
...
exits nonzero on failure so you can put it in a shell profile or a cron job. It
also runs on every push and pull request here, installing into a throwaway
`HOME` first, because the failure this repository is built around was an install
that looked complete and did nothing, so the thing worth testing is the install
...
Security issues go through [private vulnerability reporting](https://github.com/mwel10/claudevibe/security/advisories/new),
not a public issue. `SECURITY.md` says what counts, what is already known and
therefore does not need a report, and what to expect. Everything else is welcome
as an ordinary issue or pull request.
