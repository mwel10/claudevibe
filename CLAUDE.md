...
These instructions apply to every session in this project. I develop on my
own, so there is no team to catch mistakes downstream — treat Claude as the
second pair of eyes that a reviewer would normally provide.
...
- **Part A — Agent security.** Prevent credential scope, untrusted input, and
  destructive tool calls from causing an incident like Devin/Sliver, Replit,
  Amazon Q, RoguePilot, PocketOS, the Mastra npm compromise.
...
never in the global file. Use this shape:
